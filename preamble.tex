...
% \usepackage{pxfonts}
\usepackage[turkish,swedish,dutch,german,spanish,catalan,french,english,latin,irish,italian]{babel}    % Non aggiungere il giapponese, rompe la sillabazione in italiano.
\let\quoting\relax      % spanish has its own quoting command, useless for me
...
\usepackage{luacode}
\usepackage{xr-hyper}
\usepackage[a4,center,frame,cam,noinfo]{crop}  % Non per versione definitiva!

\newcommand{\mytitle}{Titolo temporaneo}
...
\newcommand{\temporaljump}[1][]{\bigskip}%
%\noindent
...
\newcommand{\catalanname}[1]{\myforeignlanguage{catalan}{#1}}%
\newcommand{\catalan}[1]{\textit{\myforeignlanguage{catalan}{#1}}}%
...
\newcommand{\ligurianname}[1]{\mbox{#1}}%
\newcommand{\ligurian}[1]{\textit{\mbox{#1}}}%
\newcommand{\provencalname}[1]{\myforeignlanguage{french}{#1}}%
\newcommand{\provencal}[1]{\textit{\myforeignlanguage{french}{#1}}}% FIXME
\newcommand{\spanishname}[1]{\myforeignlanguage{spanish}{#1}}%
\newcommand{\spanish}[1]{\textit{\myforeignlanguage{spanish}{#1}}}%
...
\newcommand{\disclaimer}{
    ...
    \textit{%
        ...
        Nomi, personaggi ed eventi sono prodotto dell'immaginazione dell’autore.
        Qualsiasi somiglianza con persone reali, vive o morte, con eventi esistenti è puramente casuale.
    }
    \end{flushleft}%
    \clearpage
}
...
\directlua{
    function smartdropcap_split(box_id)
        local n = tex.box[box_id].head
        local first = nil
        local rest = {}
        local count = 0
        while n do
            if n.id == node.id("glyph") then
                if first == nil then
                    first = unicode.utf8.char(n.char)
                else
                    count = count + 1
                    rest[count] = unicode.utf8.char(n.char)
                end
            end
            n = n.next
        end
        smartdropcap_first = first or ""
        smartdropcap_rest  = table.concat(rest)
    end
}

\NewDocumentCommand{\smartdropcapaux}{m}{%
    \setbox0=\hbox{#1}%
    \directlua{smartdropcap_split(0)}%
    \edef\firstletter{\directlua{tex.sprint(smartdropcap_first)}}%
    \edef\remainder{\directlua{tex.sprint(smartdropcap_rest)}}%
    \mylettrine{\firstletter}{\MakeLowercase{\remainder}}{\the\spokenAnteToks}%
}
...
\newcommand{\chat@format@book}[2]{
    \begingroup%
    {#1: #2}%
    \endgroup%
}%
\newcommand{\chat@format@audio}[3]{\par[#1\ifx\relax#3\relax\else, #3\fi]: #2\par}%
\newcommand{\chat}[3]{%
    \ifnum\pdf@strcmp{\detokenize\expandafter{\purpose}}{audio}=0 %
        \chat@format@audio{#1}{#2}{#3}%
    \else%
        \chat@format@book{#1}{#2}%
    \fi%
}
...
\newcommand{\chapterref}[1]{%
    \sanitize{#1}%
    \edef\tmp@target{ROMANZO-cap:\tmp@result}%
    \expandafter\hyperref\expandafter[\tmp@target]{``#1''}%
}
\makeatother

\newcommand{\chronomonth}[2]{%
\subsection{#1}%
\begin{itemize}%
#2%
\end{itemize}%
...
\newcommand{\chronochapter}[3]{\item \textbf{{#1}: ``#2''} (#3)}%

\newcommand{\chronoother}[2]{\item {\smaller {#1}: {#2}}}%
\newcommand{\chronofeast}[2]{\item {\smaller {\color{red} #1: #2}}}%
...
\newcommand{\sanitize}[1]{%
    \begingroup
    \let\xspace\empty
    \let\englishname\@firstofone
    \let\ligurianname\@firstofone
    \let\latinname\@firstofone
    \let\provencalname\@firstofone
    \let\spanishname\@firstofone
    \let\newspapername\@firstofone
    \let\protect\@unexpandable@protect
    \protected@edef\tmp@expanded{#1}%
    \StrSubstitute{\tmp@expanded}{'}{}[\tmp@a]%
    \StrSubstitute{\tmp@a}{,}{}[\tmp@a]%
    \StrSubstitute{\tmp@a}{/}{-}[\tmp@a]%
    \StrSubstitute{\tmp@a}{ }{-}[\tmp@b]%
    \StrSubstitute{\tmp@b}{--}{-}[\tmp@c]%
    \xdef\tmp@result{\tmp@c}%
    \endgroup
}
...
\renewcommand{\chapter}[1]{%
    ...
    \begingroup
    \sanitize{#1}%
    \edef\tmp@label{cap:\tmp@result}%
    \expandafter\label\expandafter{\tmp@label}%
    \endgroup
    \label{tocchap.\arabic{chapter}}% Needed by the TOC inside Note di lavoro
}
\makeatother

\makeatletter
    \newcommand{\tocwithepisodes}[1]{%
    \setlength{\cftpartindent}{0pt}
    \setlength{\cftchapindent}{0pt}
    \setlength{\cftsecindent}{10pt}
    \setlength{\cftsubsecindent}{20pt}
    \setlength{\cftsubsubsecindent}{30pt}
    \section{Sceneggiatura}%
    \setcounter{tocdepth}{4}%
    \let\originalcontentsline\contentsline
    \renewcommand{\contentsline}[4]{%
        \def\tmp@target{##4}%
        \StrSubstitute{\tmp@target}{chapter.}{tocchap.}[\tmp@target]%
        \originalcontentsline{##1}{##2}{##3}{ROMANZO-\tmp@target}%
    }%
    \begingroup
    \makeatletter
    \InputIfFileExists{#1.toc}{}{}%
    \endgroup
...
\usepackage{etoolbox}
\makeatletter
\newcommand{\keyAndValue}[2]{%
    \ifblank{#2}{}{%
    \ifblank{#1}{#2}{#1: #2}%
    }%
}
\define@key{scenemeta}{name}{\def\scenemeta@name{#1}}
\define@key{scenemeta}{setting}{\def\scenemeta@setting{#1}}
\define@key{scenemeta}{date}{\def\scenemeta@date{#1}}
\define@key{scenemeta}{time}{\def\scenemeta@time{#1}}

\newcommand{\scene}[1][]{%
    \def\scenemeta@name{}%
    \def\scenemeta@setting{}%
    \def\scenemeta@date{}%
    \def\scenemeta@time{}%
    \setkeys{scenemeta}{#1}%
    ...
    \addcontentsline{toc}{section}{
        \scenemeta@name\\
        {\small\textit{\scenemeta@date\xspace\scenemeta@time\\\scenemeta@setting}}
    }%
}
\define@key{beatmeta}{name}{\def\beatmeta@name{#1}}
\define@key{beatmeta}{driver}{\def\beatmeta@driver{#1}}
\define@key{beatmeta}{pov}{\def\beatmeta@pov{#1}}
...
\newcommand{\beat}[1][]{%
    ...
    \addcontentsline{toc}{subsection}{
        {\small\keyAndValue{pov}{\beatmeta@pov}\xspace\beatmeta@name}
    }%
}

\define@key{shotmeta}{focus}{\def\shotmeta@focus{#1}}
\define@key{shotmeta}{pov}{\def\shotmeta@pov{#1}}
\define@key{shotmeta}{sense}{\def\shotmeta@sense{#1}}
\define@key{shotmeta}{framing}{\def\shotmeta@framing{#1}}

\newcommand{\shot}[1][]{%
    \def\shotmeta@focus{}%
    \def\shotmeta@pov{}%
    \def\shotmeta@sense{}%
    \def\shotmeta@framing{}%
    \setkeys{shotmeta}{#1}%
    \refstepcounter{subsubsection}%
    \addcontentsline{toc}{subsubsection}{
        {\small\keyAndValue{}{\shotmeta@framing}\xspace\keyAndValue{}{\shotmeta@pov}\xspace\keyAndValue{
focus}{\shotmeta@focus}}
    }%
}
\makeatother

\newcommand{\sceneToday}{}

% Titolo, autore, illustrazione copertina, epigrafe, file capitoli
\newcommand{\mybook}[5]{
    \title{#1}
    \author{#2}
    \begin{document}
        \frontmatter
        \cover{#1}{#2}{#3} % FIXME: maketitle?
        \disclaimer
        \epigraph{#4}
        \mainmatter
        \input{#5}
